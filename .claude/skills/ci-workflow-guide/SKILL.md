...
A manually applied `run-ci` label is handled by the non-required `run-ci-label-listener.yml`. It verifies the event label, then reruns failed/skipped workflows from an explicit allowlist of workflows that use the live `pr-gate.yml`. Runs are matched to the exact PR and head SHA; fork runs, whose API `pull_requests` array is empty, additionally require matching head repository and branch. Workflows that already handle `labeled` or read labels from the frozen event payload are deliberately excluded.
