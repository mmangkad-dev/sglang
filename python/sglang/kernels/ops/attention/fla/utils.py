...
def input_guard(
    ...
    *,
    preserve_tensor_args: Tuple[str, ...] = (),
) -> Callable[..., torch.Tensor]:
    """Make tensor inputs contiguous, except named storage-bearing arguments."""

    def decorator(inner: Callable[..., torch.Tensor]) -> Callable[..., torch.Tensor]:
        parameters = tuple(inspect.signature(inner).parameters)
        preserved_positions = {
            parameters.index(name)
            for name in preserve_tensor_args
        ...
        def wrapper(*args, **kwargs):
            contiguous_args = (
                (
                    i
                    if index in preserved_positions or not isinstance(i, torch.Tensor)
                    else i.contiguous()
                )
            ...
            contiguous_kwargs = {
                k: (
                    v
                    if k in preserve_tensor_args or not isinstance(v, torch.Tensor)
                    else v.contiguous()
                )
            ...
            with ctx:
                return inner(*contiguous_args, **contiguous_kwargs)
