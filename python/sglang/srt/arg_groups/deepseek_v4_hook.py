...
def _validate_deepseek_v4_trtllm_attn(cfg) -> None:
    """Reject configurations the uniform-FP8 trtllm attention path cannot serve.

    ValueError, not assert: two of these guard against silent KV corruption
    (a PD peer reading the wrong cache layout, an unbounded prefill chunk
    overrunning the trtllm-gen counter buffer), so they must fire under
    ``python -O`` as well.
    """
    from sglang.srt.utils.common import is_sm100_supported

    if not (cfg.device == "cuda" and is_sm100_supported()):
        raise ValueError(
    ...
    if cfg.kv_cache_dtype not in ("auto", "fp8_e4m3"):
        raise ValueError(
            "--dsv4-attn-backend trtllm requires kv_cache_dtype=fp8_e4m3, "
            f"got {cfg.kv_cache_dtype}."
        )
    if cfg.enable_hisparse:
        raise ValueError("--dsv4-attn-backend trtllm does not support enable_hisparse.")
    ...
    # FlashMLA uses the 584-byte packed layout; the PD handshake only compares
    # kv_cache_dtype, so mismatched prefill/decode backends would pass the
    # check and transfer garbage. Reject until the handshake carries a layout
    # identifier and the path is tested.
    ...
    # The trtllm-gen counter buffer is sized from the prefill chunk bound;
    # with chunking disabled a single long request has no bound.
    if cfg.chunked_prefill_size is None or cfg.chunked_prefill_size <= 0:
        raise ValueError(
