...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Reuse tuned tactics from FlashInfer's managed autotune store under `SGLANG_CACHE_DIR/flashinfer/autotune` across runs. Set to `0` to force re-autotuning on every startup; the fresh result is published to a `runs/<timestamp>_<pid>/` store instead and the reusable one is left untouched. Set `FLASHINFER_AUTOTUNE_CACHE_DIR` to place the store elsewhere.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>`true`</td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}>`SGLANG_FLASHINFER_AUTOTUNE_MEASURE`</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>How FlashInfer measures candidate tactics: `auto` (its own per-op default), `cuda_graph` (profile under capture and replay, per-call host cost excluded as graph serving pays it once) or `eager` (host cost included). Part of the store identity, so entries measured one way are never served the other way.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>`auto`</td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}>`SGLANG_FLASHINFER_AUTOTUNE_COLD_L2`</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Force cold- or hot-L2 profiling during autotune. Unset inherits FlashInfer's per-op default. Also part of the store identity. Note: through flashinfer 0.7.0rc3, cold-L2 profiling bypasses the managed store while tuning, so setting this to `1` costs warm-start reuse for every operation.</td>
