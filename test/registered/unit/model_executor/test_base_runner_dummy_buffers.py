...
def test_allocated_dummy_buffers_run_through_dummy_forward():
    model = _Model()
    ...
    assert buffers.global_num_tokens_unpadded_gpu.shape == (1,)
    assert (
        model.forward_batch.global_num_tokens_unpadded_gpu
        is buffers.global_num_tokens_unpadded_gpu
    )
