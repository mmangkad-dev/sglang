    ...
    def test_fastsafetensors_drops_only_the_rank_owned_file(self, _):
        ...
        class FakeLoader:
            def __init__(self, group, device, max_threads, nogds):
                ...
                test_case.assertEqual(max_threads, 16)
...
class TestPrefetchDispatch(CustomTestCase):
    ...
    def test_fastsafetensors_gds_can_be_disabled(self):
        loader = self._make_loader(
            {"enable_gds": False, "max_threads": 8},
    ...
    def test_fastsafetensors_max_threads_requires_positive_integer(self):
        for max_threads in (0, -1, True, 1.5, "8"):
            with self.subTest(max_threads=max_threads):
                with self.assertRaisesRegex(
                    ValueError, "max_threads.*must be a positive integer"
                ):
                    self._make_loader(
